	...
	$.ajax({
		...
		success: function(r) {
			...
			if(r.error) {
				...
				ovault_cache.ovault_searchFunction = ovault_searchFunction;
				
				target.html(''); //remove old content
				
				if (ovault_searchFunction=='searchresults') {
				...
				} else if(ovault_searchFunction=='managesearchresults') {
					for(var i in r.resultarr) { //add to dom
						target.append(addOfferTableRow_ManagePage(r.resultarr[i]));
					}
				}
...
function addOfferTableRow(offer, oright) {
	...
	out += '<tr class="orow';
		out += oright ? ' j_oright' : '';
	out += ' j_oid-'+offer['id']+'" data-oid="'+offer['id']+'">';
	...
	//offername
	out += '<td class="td_offername">';
	
	out += '<a class="btn ovault_importoffertonetwork j_shadowbox" data-width="940" data-height="480" href="/BevoMedia/Offers/ImportFrame.html?ajax=true&network__id='+offer['network__id']+'&o='+offer['offerContractID']+'" title="Import this offer into my network">Import this offer</a>';
	
	out += '<p>';
	
	if(offer['previewUrl']) {
		out += '<a href="'+offer['previewUrl']+'" target="_blank" title="Open offer page in a new tab">'+offer['title']+'<span class="icon icon_ovault_visiticon_small"></span></a>';
	} else {
		out += offer['title']+'<span class="icon"></span>';
	}
	
	out += '</p>'+offer['offer__id']+' - '+offer['dateAdded']+'</td>';
	...
	out += '<td class="td_network" colspan="';
		out += oright ? '' : '2';
	out += '"><p class="icon">'+offer['networkName']+'</p>';
	
		//detail olay
		out += '<div class="ovault_olay olay_odetail" title="&nbsp;"><div class="olaytop"></div><div class="olaycont">';
		out += '<h3>'+offer['title']+'</h3><div class="floatleft"><div class="ovault_othumb">';
		out += offer['imageUrl'] ? '<img src="'+offer['imageUrl']+'" alt="" />' : '<img src="/Themes/BevoMedia/Images/othumb_default.gif" alt="" />';
		out += '</div></div><!--close floatleft--><div class="floatright">';
		
		out += '<div class="ratingstars">'+offer['userRating']+'</div><div class="clear"></div>';
		out += '<div class="otitle otitle_offerdesc_yell"></div><p>'+offer['detail']+'</p>';
		out += '</div><!--close floatright--><div class="clear"></div></div><!--close olaycont--></div><!--close olay-->';
...
function addOfferTableRow_ManagePage(offer, oright) {
	...
	if (ovault_networdIntegrated=='1')
	{
		out += '<select class="formselect showSelect" OfferID="'+offer.id+'" id="showSelect_'+offer.id+'" name="showSelect_'+offer.id+'">';
		out += '<option value="0" '+((offer.hidden==0)?'selected':'')+'>Show In Affiliate Portal and Offer Import</option>';
		out += '<option value="2" '+((offer.hidden==2)?'selected':'')+'>Show in Affiliate Portal Only</option>';
		out += '<option value="3" '+((offer.hidden==3)?'selected':'')+'>Show in Offer Import Only</option>';
		out += '<option value="1" '+((offer.hidden==1)?'selected':'')+'>Hide from All</option>';
		out += '</select>';
	} else
	{
		out += '<select class="formselect showSelect" OfferID="'+offer.id+'" id="showSelect_'+offer.id+'" name="showSelect_'+offer.id+'">';
		out += '<option value="0" disabled>Show For In Affiliate Portal and Offer Import</option>';
		out += '<option value="0" >Show in Affiliate Portal Only</option>';
		out += '<option value="3" disabled>Show in Offer Import Only</option>';
		out += '<option value="1" '+((offer.hidden==1)?'selected':'')+'>Hide from All</option>';
		out += '</select>';
	}
	...
	out += offer.title;
	out += '</td>';
	out += '<td class="center">'+offer.offerType+'</td>';
	out += '<td class="center">'+offer.payout+'</td>';
	out += '<td class="center">Active</td>';
	out += '<td class="small center">'+offer.categoryTitle+'</td>';	
	out += '<td class="center">';
	out += '<a class="tbtn small j_shadowbox" data-height="540" href="/BevoMedia/Offers/EditOffer.html?ID='+offer.id+'">Edit</a>';
	...
	if(resultarr == 'nolists') {
		...
		if(listdata) {
			...
			if(listdata.num_offers >= 1)
				$('#oright .conttop .top4').append('<a class="tbtn small" href="?ExportTo=CSV">Export to CSV</a><div class="clear"></div>');			
		}
